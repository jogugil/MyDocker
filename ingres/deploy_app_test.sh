...
limpiar_entorno() {
  ...
  if [ "$respuesta" == "s" ]; then
    ...
    microk8s kubectl delete ingress nfs-ingress
...
aplicar_recursos(){
  ...
  # Crear los Ingress
  microk8s kubectl apply -f nfs-ingress.yaml
  echo "Recurso nfs-ingress.yaml aplicado correctamente."
...
verificar_pods() {
  ...
  while [ $tiempo_transcurrido -lt $tiempo_maximo ]; do
    microk8s kubectl get endpoints nginx-service    # Obtenemos el estado detallado del Pod Apache
...
verificar_servicios() {
  ...
  microk8s kubectl describe service apache
  microk8s kubectl descrie service nginx
  microk8s kubectl get endpoints apache-service nginx-service
}
...
verificar_ingress() {
  ...
  microk8s kubectl describe ingress nfs-ingress
}
