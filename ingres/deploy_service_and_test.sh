...
aplicar_recursos(){
  ...
  # Crear los servicios
  microk8s kubectl apply -f nfs-service.yaml
  echo "Recurso nfs-service.yaml aplicado correctamente."
...
probar_acceso() {
  ...
  curl -v http://pgc-alu.info/apache
  echo -e "\nProbando acceso a Nginx..."
  curl -v http://pgc-alu.info/nginx
}
